...
class ReportViewSet(viewsets.ViewSet):
    ...
    @extend_schema(responses=inline_serializer(name='SalesSummaryResponse', fields={'summary': serializers.DictField(), 'daily_breakdown': serializers.ListField(child=serializers.DictField())}))
    ...
    def top_products(self, request):
        ...
        top_items = SaleItem.objects.values('product__id', 'product__name', 'product__product_type__name').annotate(total_quantity=Sum('quantity'), total_revenue=Sum(F('quantity') * F('unit_price'))).order_by('-total_quantity')[:limit]
    ...
    def stock_alerts(self, request):
        ...
        low_stock = Product.objects.filter(stock_quantity__lte=low_threshold).values('id', 'name', 'stock_quantity', 'base_unit', 'product_type__name')
        expiring_soon = Product.objects.filter(expiration_date__lte=expiry_cutoff_date, expiration_date__gt=timezone.now().date()).values('id', 'name', 'expiration_date', 'stock_quantity', 'product_type__name')
        expired = Product.objects.filter(expiration_date__lt=timezone.now().date()).values('id', 'name', 'expiration_date', 'stock_quantity', 'product_type__name')
